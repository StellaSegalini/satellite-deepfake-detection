...
OUTPUT_BASE_DIR = "/oblivion/users/ssegalini/tesi_satellitare/data/fake_test2"
...
# 15 prompt totali * 40 = 600 immagini fake di test (120 per macro-categoria)
...
def main():
    ...
    print("Inizio generazione del dataset Fake di TEST (600 campioni unseen)...")
    
    img_counter = 0
    total_expected = sum(len(prompts) * IMAGES_PER_PROMPT for prompts in PROMPT_CATEGORIES.values())
    pbar = tqdm(total=total_expected, desc="Generazione Fake Unseen")
    ...
    for category, prompts in PROMPT_CATEGORIES.items():
        ...
        for p_idx, prompt in enumerate(prompts):
            for i in range(IMAGES_PER_PROMPT):    
                current_seed = SEED_OFFSET + img_counter
    ...
    pbar.close()
    print(f"\nCompletata generazione di {img_counter} nuove immagini fake in: {OUTPUT_BASE_DIR}")
